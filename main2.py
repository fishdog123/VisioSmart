...
from config import (
    ...
    llm_one_shot_queue, CHAT_MODE, SCENE_MODE,
)
...
from modes import CurrencyDetector, FaceRecognizer, OCRProcessor, ObjectDetector
...
def preload_all():
    def _load():
        ...
        print("[INFO] Loading Scene Description...")
        try:
            processors[SCENE_MODE] = SceneDescriber(processors)
            print("[INFO] ✓ Scene Description loaded successfully")
        except Exception as e:
            print(f"[ERROR] Failed to load Scene Description: {e}")
            failed_modes[SCENE_MODE] = str(e)
...
def main():
    ...
    try:
        while True:
            ...
            if active_mode == SCENE_MODE:
                try:
                    processors[SCENE_MODE].run_once(frame)
                except Exception as e:
